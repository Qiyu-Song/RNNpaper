...
if __name__ == '__main__':
    ...
    # Two-stage training (reversed)
    # Stage 1: train with both random forcing data and coupled-wave forcing data
    # Stage 2: train with random forcing data
    ...
    # define model name
    modeltype = f"noisywave1.0_6to20_damp2_seqlen_{seq_len}_skip_{skip_wave}"
    if stage == 2:
        modeltype = modeltype + f"_rand_4_to_50_seqlen_{seq_len}_skip_{skip_rand}"
    if h0_only:
    ...
    if stage == 1 and (not h0_only):
        loadmodel = './logs/noisywave1.0_6to20_damp2_seqlen_192_skip_24_h0_1e-04_256/version_0/checkpoints/epoch=999-step=14000.ckpt'
    elif stage == 2:
        if h0_only:
            loadmodel = './logs/noisywave1.0_6to20_damp2_seqlen_192_skip_24_all_param_1e-05_128/version_2/checkpoints/epoch=9959-val_loss=0.077.ckpt'
        else:
            loadmodel = ''
    ...
    if n_rand_exp > 0:
        for idx in range(len(val_h0_idx_rand)):
            data = SAMdata[idx + len(val_h0_idx_wave)]
            inputs_valid_exp = data.input.unsqueeze(1)
            targets_valid_exp = data.target.unsqueeze(1)
    ...
    valid_dataset = TensorDataset(torch.from_numpy(np.array(val_h0_idx_wave + val_h0_idx_rand)),
                                  torch.from_numpy(np.array(val_h0_idx_wave + val_h0_idx_rand)))
    valid_loader = DataLoader(valid_dataset, batch_size=len(val_h0_idx_wave) + len(val_h0_idx_rand), shuffle=False,
